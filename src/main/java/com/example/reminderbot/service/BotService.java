...
public class BotService {
    ...
    private static final Duration RE_PING_DELAY = Duration.ofMinutes(30);
    private static final Duration WORK_CHECK_DELAY = Duration.ofMinutes(30);
        ...
        synchronized (this) {
            ...
            for (int i = 0; i < state.prompts().size(); i++) {
                ...
                ActivePrompt updated = new ActivePrompt(
                        ...
                        nextState,
                        prompt.messageId(),
                        prompt.alertBroadcastCount() + (shouldBroadcast ? 1 : 0),
                        prompt.endOfDayAlertSent()
                );
                state.prompts().set(i, updated);
    ...
    private void handleMessage(TelegramClient.Message message) {
        ...
        if (text.equals("/alerts")) {
            telegram.sendMessage(profile.chatId(), alertsText(profile.chatId()), alertsKeyboard());
            return;
        }
        if (text.equalsIgnoreCase("/alerts on")) {
            setAlertsEnabled(profile.chatId(), true);
            telegram.sendMessage(profile.chatId(), "Ты подписан на общие алерты по пропущенным делам.");
            return;
        }
        if (text.equalsIgnoreCase("/alerts off")) {
            setAlertsEnabled(profile.chatId(), false);
            telegram.sendMessage(profile.chatId(), "Ты отписан от общих алертов.");
            return;
        }
    ...
    private void handleCallback(TelegramClient.CallbackQuery callback) {
        ...
        try {
            ...
            if (data.startsWith("SUB_DAILY_HOUR:")) {
                ...
                telegram.answerCallbackQuery(callback.id(), "Час сохранён");
                return;
            }
            ...
            if (data.startsWith("PROMPT_GO:")) {
                handlePromptGoDoing(callback.id(), data.substring("PROMPT_GO:".length()), chatId);
                return;
            }
    ...
    private void handleTaskCard(long chatId, String ref) {
        ...
        String text = """
                ...
                subscribers,
                own == null ? "У тебя пока нет настройки для этого дела." : ("Твоя настройка: " + scheduleHuman(own, task))
        );
        ...
        if (task.kind() == TaskKind.MANUAL) {
            ...
            rows.add(List.of(TelegramClient.button(own == null ? "Настроить" : "Изменить настройку", "SUB_START:" + number)));
            if (own != null) {
                rows.add(List.of(TelegramClient.button("Удалить настройку", "UNSUB:" + number)));
            }
        }
        rows.add(List.of(TelegramClient.button("Кто подписан", "WHO:" + number)));
    ...
    private void sendDailyHourPicker(long chatId, TaskDefinition task) {
        ...
        String current = existing == null || existing.dailyTimes().isEmpty()
                ? "Пока часов нет."
                : "Сейчас: " + String.join(", ", existing.dailyTimes());
        String text = """
                %s

                Выбери час. Для нескольких напоминаний в день просто нажми ещё один час ещё раз.
                %s
    ...
    private void handleDailyHourPick(long chatId, String taskId, int hour) {
        ...
        synchronized (this) {
            ...
            hours.add(picked.format(TIME_SHORT));
        ...
        telegram.sendMessage(chatId, "Сохранил: " + task.title() + "\n" + currentSubscriptionShort(chatId, task.id()));
    }
    ...
    private void sendTasksPage(long chatId, int page) {
        ...
        StringBuilder sb = new StringBuilder("Дела — страница ")
                .append(safePage + 1).append("/").append(totalPages).append("\n\n");
        ...
        sb.append("Нажми на кнопку с номером или напиши /task <номер>.");

        List<List<Map<String, String>>> rows = new ArrayList<>();
        for (int i = from; i < to; i++) {
            int number = i + 1;
            rows.add(List.of(TelegramClient.button(number + ". " + trimTitle(tasks.get(i).title()), "TASK_SHOW:" + number)));
        }
    ...
    private void handlePromptGoDoing(String callbackId, String promptId, long chatId) {
        synchronized (this) {
            ...
            replacePrompt(new ActivePrompt(
                    ...
                    prompt.chatId(),
                    prompt.scheduledFor(),
                    Instant.now().plus(WORK_CHECK_DELAY),
                    STATE_IN_PROGRESS,
                    prompt.messageId(),
                    prompt.alertBroadcastCount(),
                    prompt.endOfDayAlertSent()
        ...
        telegram.sendMessage(chatId, "Хорошо, перепроверю через 30 минут.");
    }
            ...
            synchronized (this) {
                ...
                if (current != null) {
                    replacePrompt(new ActivePrompt(
                            ...
                            current.state(),
                            messageId,
                            current.alertBroadcastCount(),
                            current.endOfDayAlertSent()
                    ));
                    saveState();
                }
            }
    ...
    private void sendGlobalAlert(ActivePrompt prompt, TaskDefinition task, AlertReason reason) {
        ...
        for (UserProfile profile : new ArrayList<>(state.users().values())) {
            ...
            telegram.sendMessage(profile.chatId(), text, alertsKeyboard(), false);
        }
    }
    ...
    private String startText() {
        return """
                ...
                Как пользоваться:
                1) Нажми «Список дел» или /tasks
                2) Открой карточку нужного дела
                3) Нажми «Настроить» и выбери дату/час кнопками
                4) Когда придёт пинг, нажми «Сделал», «Пошёл делать» или «Отложить»

                Если не ответить, бот перепингует через 30 минут.
Если долго игнорировать задачу, бот поднимет общий алерт всем подписанным на алерты.
                """;
    }
    ...
    private String helpText() {
        return """
                Что умеет бот
                • хранит список дел из JSON
                • настраивает напоминания без технических ID
                • даёт выбрать дату по кнопочному календарю
                • даёт выбрать час кнопками
                • напоминает повторно каждые 30 минут, если ты не ответил
                • кнопка «Пошёл делать» автоматически запускает проверку через 30 минут
                • если дело игнорируют, бот шлёт общий алерт всем, кто не отключил алерты
                • если дело не закрыто до конца дня, бот тоже шлёт общий алерт

                Основные команды
                /tasks — список дел
                /task <номер> — карточка дела
                /sub <номер> — открыть настройку дела
                /unsub <номер> — убрать настройку
                /subs — мои настроенные дела
                /who <номер> — кто подписан на дело
                /tz — моя таймзона
                /tzset <таймзона> — сменить таймзону вручную
                /alerts — подписка на общие алерты
                /alerts on|off — включить или выключить общие алерты
                ...
                • ежедневные дела: выбираешь один или несколько часов
                • еженедельные/ежемесячные: выбираешь дату на календаре, потом час
                • разовые: выбираешь дату и час один раз
                """;
    }
    ...
    private UserProfile registerUser(TelegramClient.Message message) {
        ...
        synchronized (this) {
            ...
            String zone = existing == null || existing.zoneId() == null || existing.zoneId().isBlank()
                    ? defaultZone.getId()
                    : existing.zoneId();
            UserProfile profile = new UserProfile(chatId, username, firstName, zone, existing == null || existing.alertsEnabled());
    ...
    private UserProfile synchronizedProfile(long chatId) {
        synchronized (this) {
            ...
            return p == null ? new UserProfile(chatId, null, "User", defaultZone.getId(), true) : p;
        }
    }
