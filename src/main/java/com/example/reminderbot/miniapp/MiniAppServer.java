...
public class MiniAppServer {
    ...
    private String html(Map<String, String> q) {
        ...
        return "<!doctype html>\n" +
                ...
                "<div class=\"muted\">Тип: " + esc(q.getOrDefault("kind", "RECURRING")) + " · правило: " + esc(q.getOrDefault("unit", "DAY")) + " / каждые " + interval + " · зона: " + zone + "</div>\n" +
                ...
                "if (kind === 'RECURRING' && unit === 'DAY') { dailyBlock.classList.remove('hidden'); } else { datedBlock.classList.remove('hidden'); document.getElementById('date').value = todayIso(); }\n" +
                ...
                "document.getElementById('save').onclick=()=>{ let payload; if(kind==='RECURRING' && unit==='DAY'){ if(!times.length && timeInput.value) times.push(timeInput.value); if(!times.length){ alert('Добавь хотя бы одно время'); return; } payload={type:'subscription',taskId,mode:'daily',times:[...times].sort()}; } else { const date=document.getElementById('date').value; const time=document.getElementById('time').value; if(!date || !time){ alert('Выбери дату и время'); return; } payload={type:'subscription',taskId,mode:'dated',date,time}; } tg.sendData(JSON.stringify(payload)); tg.close(); };\n" +
                "</script>\n</body>\n</html>\n";
    }
